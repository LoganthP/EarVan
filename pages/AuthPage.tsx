      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-3xl shadow-xl overflow-hidden border border-slate-100 dark:border-slate-700 transition-all duration-300">
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input
              label={mode === 'SIGNUP' ? "Username" : "Email Address"}
              name={mode === 'SIGNUP' ? "username" : "email"}
              placeholder={mode === 'SIGNUP' ? "johndoe123" : "john@example.com"}
              value={mode === 'SIGNUP' ? formData.username : formData.email}
              onChange={handleChange}
              icon={mode === 'SIGNUP' ? <UserIcon className="w-5 h-5" /> : <Mail className="w-5 h-5" />}
            />
